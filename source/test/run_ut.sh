...
log "INFO" "Running make for CcspMtaAgentSsp_gtest.bin..."
...
if [ -f "./source/test/CcspMtaAgentSsp_gtest.bin" ]; then
    log "INFO" "Running CcspMtaAgentSsp_gtest.bin"
    ./source/test/CcspMtaAgentSsp_gtest.bin
    log "INFO" "Completed Test Execution"
else
    log "ERROR" "CcspMtaAgentSsp_gtest.bin does not exist, cannot run tests"
    exit 1
fi
